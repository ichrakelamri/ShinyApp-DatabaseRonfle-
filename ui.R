    ...
    tabPanel("Data",
             ...
             sidebarLayout(
               ...
               mainPanel(
                 ...
                 p("This App shows the application of 3 different classification algorithms (rpart, randomForest and lda) to the ronfle dataset."),
                 ...
                 p("Lastly it trains a classification algorithm chosen by the user on the sidepar panel, splitting the ronfle dataset in training and testing sets based on the chosen proportion chosen on the sidebar (by defuault 70% training 30% testing)."),
    ...
    tabPanel("L'analyse Bi-variee", 
             sidebarPanel(
               ...
               conditionalPanel( condition = "input.type1 == 'Quantitative'",
                                 selectInput("vquanti1","Choisir la variable ~ RONFLE",
                                             choices = c(colnames(ronfle[c(1,2,3,4)])) 
                                 )
               ),
               conditionalPanel( condition = "input.type1 == 'Qualitative'",
                                 selectInput("vquali1","Choisir la variable ~ RONFLE",
                                             choices = c(colnames(ronfle[-c(1,2,3,4)])) 
                                 ))
             ),
             mainPanel(
